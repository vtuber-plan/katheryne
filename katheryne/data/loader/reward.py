...
def load_reward_messages(dataset_name: str, field: Union[str, List[str]], field_map: Dict[str, str]={}, split:str="train", data_dir=None, data_files=None):
...
def create_dataset(dataset_name) -> Tuple[datasets.Dataset, datasets.Dataset]:
    ...
    if "train" in raw_datasets:
        raw_train_dataset = load_reward_messages(dataset_name, ["prompt", "chosen", "rejected"], split="train")
    else:
        raw_train_dataset = None

    if "validation" in raw_datasets:
        raw_validation_dataset = load_reward_messages(dataset_name, ["prompt", "chosen", "rejected"], split="validation")
    elif "valid" in raw_datasets:
        raw_validation_dataset = load_reward_messages(dataset_name, ["prompt", "chosen", "rejected"], split="valid")
    elif "eval" in raw_datasets:
        raw_validation_dataset = load_reward_messages(dataset_name, ["prompt", "chosen", "rejected"], split="eval")
    elif "evaluation" in raw_datasets:
        raw_validation_dataset = load_reward_messages(dataset_name, ["prompt", "chosen", "rejected"], split="evaluation")
    else:
