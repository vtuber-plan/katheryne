...
def get_subset(seed: int, dataset, splits: List[float]):
    data_size = len(dataset)
    shuffle_idx = get_shuffle_idx(seed, data_size).tolist()
    out = []
    last_pos = 0
    for i, split in enumerate(splits):
        split_len = int(split*data_size)
        idx = shuffle_idx[last_pos:last_pos + split_len]
        last_pos += split_len
        out.append(idx)
    return out
    
    sub_dataset = Subset(dataset, shuffle_idx[split_pos:])
    ...
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(tqdm(current_dataset)):
            ...
            if chosen_sentence is not None and reject_sentence is not None:
                ...
                chosen_token = tokenizer(chosen_sentence, max_length=max_seq_len, padding=False, truncation=True, return_tensors="pt")
                reject_token = tokenizer(reject_sentence, max_length=max_seq_len, padding=False, truncation=True, return_tensors="pt")
